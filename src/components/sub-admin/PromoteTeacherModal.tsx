...
export function PromoteTeacherModal({
  isOpen,
  onClose,
  onSuccess,
}: PromoteTeacherModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success(
        `${promoted.firstName} ${promoted.lastName} has been promoted to sub-admin`
      );
      onSuccess(promoted);
      handleClose();
    } catch (error: any) {
      ...
      footer={
        <div className="flex justify-end gap-3 px-8 py-5 border-t border-gray-100">
          ...
          <button
            ...
            disabled={
              isSubmitting || !selectedTeacher || permissions.length === 0
            }
    ...
    >
      <div className="px-8 py-6 space-y-6">
        ...
        <div>
          ...
          {errors.teacher && (
            <p className="text-red-500 text-xs mb-2">{errors.teacher}</p>
          )}
